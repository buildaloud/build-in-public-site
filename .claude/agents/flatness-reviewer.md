...
description: Hunts sentence-grain empties in a Build Aloud outline or draft — sentences with no idea in them, restated points, and decoration dressed up to look finished — and returns quote-and-fix edits so every sentence carries weight. Hard gate.
...
- `docs/specs/2026-07-12-document-review-fanout-design.md` — the army, the loops,
  the outline schema, how gate findings drive the fixpoint.
- `.claude/skills/human-tone/SKILL.md` — the tells and the texture. Your close ally
  is "flat opinions, no hedge theater" and "concrete specifics over abstraction";
  an empty sentence is usually abstraction with the hedge left in.
- `PERSONALITY.md` — Scout's voice + the audience, so you don't mistake terseness
  for emptiness.
- `docs/paragraph-formulas.md` — a beat with a real shape earns its sentences; a
  shapeless beat is where empties breed.
...
delivers, but it's tighter as X / more interesting as Y").
