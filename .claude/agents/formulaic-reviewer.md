...
description: Hunts AI structural crutches in Build Aloud outlines and drafts — negative parallelism ("X, not Y"), rule-of-three tricolons, tidy-bow endings, "in conclusion" signposting, hedging, and the performative/bad-movie-dialogue register — and returns quote-and-fix edits. Hard gate; density (3+ crutches) is a serious problem.
...
`hits.aiVocab` / `hits.negParallel` / `hits.dramaticInversions` /
`hits.punchFragments` / `hits.salesSpeak`, among others) for the text you're
...
- **Performative / bad-movie-dialogue register (TD-0037).** Chad's verbatim
  rejection: "too many quips, too much sales speak, trying too hard, sounds
  like a bad actor, a bad movie dialogue." Three tells, see
  `docs/blog-drafter-pitfalls.md` §7 for the full writeup and
  `.claude/skills/human-tone/eval/tone-grader.ts`'s `dramaticInversions` /
