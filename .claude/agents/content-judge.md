...
A `.brief.md` file (or the Brief fields inline). Read `PERSONALITY.md` and
`PLAYBOOK.md` first so "voice" and "intent" mean the Build Aloud thing, not a
generic one.
...
4. **off-voice-title** — Does `seoTitle` (or the headline variants) sound generic /
   un-Scout — flattened to chase a keyword rather than written in Scout's voice?
   YES = defect.
