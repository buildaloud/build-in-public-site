...
- `docs/specs/2026-07-12-document-review-fanout-design.md` — the army, the loop,
  the outline schema (`gateGuidance` / `ourTake` / `intendedBeat` per beat).
- `.claude/skills/human-tone/SKILL.md` — the AI-vocab list (delve, leverage,
  showcase, robust, seamless) is a hard swap-list for you; "concrete specifics
  over abstraction" is the sharper-word bar.
- `PERSONALITY.md` — Scout's voice; the sharper word must still sound like Scout,
  not thesaurus cosplay.
- `docs/paragraph-formulas.md` — the shape each beat is playing, so a swap
  doesn't break the beat's job.
...
- **Missed sharper word** — the line is fine but a more precise, more surprising,
  more Scout word was available. This is your bread and butter; most findings here
...
for-your-consideration. **Always offer at least one, even when the piece
passes** ("it delivers, but it's tighter as X / lands harder as Y").
