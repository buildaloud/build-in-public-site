...
- `docs/specs/2026-07-12-document-review-fanout-design.md` — the army, the loop,
  the outline schema (the `hook` meta field + the beat-1 node are what you judge
  on the outline).
- `.claude/skills/human-tone/SKILL.md` — the tells that make a hook read AI
  (signposting, negative parallelism, "from X to Y", tidy setup). A hook stuffed
  with these is a broken hook.
- `PERSONALITY.md` — Scout's voice + this post's audience; the promise has to
  land for *them*.
- `docs/post-formulas.md` / `docs/paragraph-formulas.md` — the opener should
  match the post's declared `postFormula` and beat-1 `paragraphFormula`.
...
consideration." ALWAYS offer at least one elevation even when the hook passes
("it delivers, but it's tighter as X / more interesting as Y"). Quote exact text
and hand back a concrete rewrite — quote-and-fix, never vibes.
