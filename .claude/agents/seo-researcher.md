...
2. **PLAYBOOK** — `/Users/chadfurman/projects/build-aloud/PLAYBOOK.md` (Section 4 keyword-theme clusters, Section 3 voice > SEO rule).
3. **Cannibalization check inputs** — `src/content/blog/*.md` frontmatter (read `targetKeyword`/`secondaryKeywords`; fall back to `title`/`tags` for legacy posts that predate SEO frontmatter) and `.plans/drip-plan.md`.
...
5. Run the cannibalization check **once**, across all candidates together: flag any opportunity keyword that overlaps an existing post's `targetKeyword`/`secondaryKeywords` (or, for legacy posts, its `title`/`tags`) or a `.plans/drip-plan.md` entry.
...
keywordOpportunitySet:
  candidates:
    - seedTopic: "string"
      playbookCluster: "one PLAYBOOK §4 cluster"
  ...
  cannibalization:
    - keyword: "string"
      conflictsWith: "existing post slug or drip-plan entry"
  caveat: "heuristic until posts carry SEO frontmatter"
```

This matches the `KeywordOpportunitySetSchema` at `.claude/skills/content-pipeline/lib/keyword-opportunity-schema.ts` — no numeric-volume field exists on that schema, so a fabricated volume cannot round-trip.
...
3. **Research digest** — read `/Users/chadfurman/projects/build-aloud/.plans/drip-plan.md` and extract the entry for this topic (context, angle, what's already known).
4. **PLAYBOOK** — read `/Users/chadfurman/projects/build-aloud/PLAYBOOK.md` (Section 4 keyword-theme clusters, Section 3 voice > SEO rule).
...
The Step 2 gate only captures a **topic** choice, not a per-keyword pick — a per-keyword override (Chad choosing a specific opportunity within the approved topic, not just the topic itself) is **NOT YET SUPPORTED** (future work).
...
- **No cannibalization:** the target keyword must not duplicate a keyword already owned by an existing post. Check `.plans/drip-plan.md` for what's already in the pipeline. (This is the fallback path only — when discovery ran, cannibalization was already checked once at that gate; do not re-run it here.)
