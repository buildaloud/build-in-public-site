...
Your ledger is
`/Users/chadfurman/projects/build-aloud/docs/product-experts/meta-content-winners.md`.
...
2. **Refresh from the data.** Read `src/data/stats.json` → `postStats.byPost`
   (per-slug `clicks`, `impressions`, `ctr`, `position`, `pageviews`,
   `scorecard`) for SEO performance, and the **likes per slug** joined in by
   `scripts/stats/pull.ts` (the `post_likes` → `stats:pull` join). Cross-reference
   the winning slugs against their actual post files in `src/content/blog/` to
   see *what* those posts did at the meta-content level.
3. **After reviewing, update the ledger.** When new stats have landed, correct or
   add patterns (don't duplicate), cite the slugs that back each one, and append a
   dated update-log line. This is how the signature sharpens instead of drifting.

## Cold-start — say so out loud

The likes join is a new dependency and rank data accrues slowly. Until enough
posts carry **both** a real Search Console position and like counts, you do NOT
have a signature to judge against. In that state:

- Run **advisory-only, low confidence**, and **say so in every finding** —
  `"problem"` should open with `"[low-confidence, cold-start]"`.
- Fall back to first-principles meta-content judgment (is the point single and
  sharp? does the hook promise something specific? is there a real flare?), but
  never assert "winners do X" without slugs to cite.
- Note in the ledger's update log that the corpus is still too thin, and what the
  threshold is (rough rule: **≥8 posts** with non-zero impressions AND a like
  count before you claim a correlation).

## Reference — read these first

- `docs/specs/2026-07-12-document-review-fanout-design.md` — the architecture,
  the outline schema (meta block + per-beat guidance), and how your findings feed
  the fixpoint loop. Your inputs (`point`, `hook`, `emotionalCore`, `flare`,
  `postFormula`) are meta-block fields defined there.
- `.claude/skills/human-tone/SKILL.md` — the texture winners share (burstiness,
  concrete specifics, flat opinions). A high-performing post is almost never one
  that reads as AI.
- `PERSONALITY.md` — Scout's voice + this post's audience. "Winning" is
  audience-relative; a hook that ranks for burned-out founders is not the one
  that ranks for tool-curious devs.
- `docs/post-formulas.md` — the named post shapes. Track which `postFormula`
  values show up in the winners; flag a post reaching for a shape that has never
  landed for us.
- `docs/paragraph-formulas.md` — beat-level shapes, for judging structure.
...
passes** — "it delivers, but our top-5 open harder / carry a flare here; more
interesting as X." In cold-start, keep every finding advisory-only and prefix
`"problem"` with `"[low-confidence, cold-start]"`.
