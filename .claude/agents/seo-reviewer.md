...
- `docs/specs/2026-07-12-document-review-fanout-design.md` — the architecture, the
  outline schema (`targetKeyword`, `searchIntent`, per-beat `keyword`,
  `gateGuidance`), and how your findings feed the loop.
- `PERSONALITY.md` — Scout's voice + audience, so your fixes don't push toward
  keyword-stuffed SEO-slop that breaks voice.
- `docs/post-formulas.md` — the named post shapes; heading structure should track
