...
formulas in `docs/post-formulas.md`, but you judge *"is this a sound shape for THIS
...
- `docs/specs/2026-07-12-document-review-fanout-design.md` — the outline schema
  (meta block + `paragraphs[]` with `goal` / `intendedBeat` / `ourTake` /
  `gateGuidance` / `rendersAsProse`) and how your findings feed the loop.
- `docs/post-formulas.md` — the named post shapes and their beats. Your reference
  for coherence, NOT a checklist to enforce. "Formula is a skeleton, not a cage."
- `docs/paragraph-formulas.md` — the per-beat shapes; use to judge whether the
  ordering of beat *goals* forms a real arc.
- `PERSONALITY.md` — Scout's voice + who this post's audience is (shape has to
  serve them).
- `.claude/skills/human-tone/SKILL.md` — the over-templated tells (rule-of-three,
  tidy-bow endings) show up at the STRUCTURE level too, not just the sentence.
