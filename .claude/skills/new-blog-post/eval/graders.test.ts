...
const goodBrief: Brief = {
  ...
  internalLinks: ['https://buildaloud.ai/something', '/blog/intro-to-agents'],
  cta: 'Try it now',
  socialBlurb: 'Building ai agents in public. buildaloud.ai/blog/ai-agents',
...
describe('gradeSchema', () => {
  ...
  it('fails when internalLinks has fewer than 2 items', () => {
    expect(gradeSchema({ ...goodBrief, internalLinks: ['https://buildaloud.ai'] }).pass).toBe(false);
  });
...
describe('gradeInternalLinks', () => {
  ...
  it('fails when an absolute URL host is off-allowlist', () => {
    const brief = { ...goodBrief, internalLinks: ['https://buildaloud.ai/a', 'https://evil.com/page'] };
  ...
  it('passes with marketplace subdomain link', () => {
    const brief = { ...goodBrief, internalLinks: ['https://marketplace.buildaloud.ai/skill', '/blog/foo'] };
    expect(gradeInternalLinks(brief).pass).toBe(true);
  });

  it('passes with td subdomain link', () => {
    const brief = { ...goodBrief, internalLinks: ['https://td.buildaloud.com/game', '/blog/foo'] };
    expect(gradeInternalLinks(brief).pass).toBe(true);
  });

  it('passes with /projects relative path', () => {
    const brief = { ...goodBrief, internalLinks: ['https://buildaloud.ai/page', '/projects'] };
    expect(gradeInternalLinks(brief).pass).toBe(true);
  });

  it('passes with /projects/ nested path', () => {
    const brief = { ...goodBrief, internalLinks: ['/blog/post', '/projects/tower-defense'] };
    expect(gradeInternalLinks(brief).pass).toBe(true);
  });

  it('fails with an off-site relative path', () => {
    const brief = { ...goodBrief, internalLinks: ['https://buildaloud.ai/a', '/about'] };
    expect(gradeInternalLinks(brief).pass).toBe(false);
  });
});
...
describe('gradeBannedTerms', () => {
  ...
  it('fails when "change-factory" appears in topic', () => {
    expect(gradeBannedTerms({ ...goodBrief, topic: 'Using the change-factory approach' }).pass).toBe(false);
  });

  it('fails when "change factory" (no hyphen) appears in hook', () => {
    expect(gradeBannedTerms({ ...goodBrief, hook: 'The change factory is a methodology' }).pass).toBe(false);
  });

  it('is case-insensitive for banned terms', () => {
    expect(gradeBannedTerms({ ...goodBrief, cta: 'Try Change-Factory today' }).pass).toBe(false);
  });

  it('fails when banned term appears in outline item', () => {
    expect(gradeBannedTerms({ ...goodBrief, outline: ['What is it', 'Change-Factory pattern', 'How to use', 'Summary'] }).pass).toBe(false);
  });
});
