...
export interface ToneMetrics {
  ...
  dramaticInversions: number; // "By the time X, Y had already Z" / "N things run before X" hook shapes (TD-0037)
  punchFragments: number; // prose (non-heading/list) ≤3-word sentences ending in . or ! (TD-0037)
  salesSpeak: number; // turbocharge/supercharge/unlock-as-verb/pitch-deck cadence (TD-0037)
...
// Permabanned phrases (TD-0027): hollow AI tics Chad never wants shipped. A single
...
// TD-0037: performative/bad-movie-dialogue register — Chad's rejected feedback,
// verbatim: "too many quips, too much sales speak, trying too hard, sounds like
// a bad actor, a bad movie dialogue." Three families below.
...
// (confirmed against the human corpus + shipped posts during calibration;
// see calibration notes in the TD-0037 report).
...
export function scoreText(raw: string): ToneMetrics {
  ...
  // Calibrated 2026-07-15 against the shipped blog corpus (TD-0037): Scout's
  // established burstiness texture (SKILL.md "jam a fragment against a run")
  // legitimately produces 2-25 short sentences per post (median ~7) as normal
  // style, not register drift. The dispatched weight+cap (free=2, +4, cap 16)
  // blanket-failed 30/52 clean posts; free=10 tracks the observed normal
  // ceiling (matches the dispatched literal "some are good, density is the
  // tell" framing) — 0/52 clean posts newly cross the aiScore<15 gate at
  // free=10/weight=1/cap=6, while genuine outliers (17, 25 raw hits) still
  ...
  score += bannedHits.length * 100; // permabanned phrases (TD-0027): any hit is a hard fail
