...
This is a genuine `war-story` shape (the "custom domain blocks Pages
Functions" incident, drawn from this repo's real history), and it matches the
formula's beats in `docs/post-formulas.md` one-to-one:
...
- **False lead** (order 2) — a named wrong assumption that cost real time
  ...
  confused."
- **The reveal** (order 3) — the actual root cause, named plainly (a zone
  Worker Route shadowing `/api/*`), explicitly flagged in `gateGuidance` as
