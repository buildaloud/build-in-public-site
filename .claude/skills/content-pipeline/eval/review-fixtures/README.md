...
(`docs/specs/2026-07-12-document-review-fanout-design.md`). Each fixture pairs
...
Real content throughout, no lorem ipsum — most fixtures reuse or lightly
adapt actual incidents/decisions from this repo's own history (the Cloudflare
custom-domain routing bug, the ticket-kit link-map drift, the Supabase like
counter) so the fixtures double as regression anchors for real defects this
...
All six are **gate**-disposition per the design doc's reviewer table — these
...
converges. Advisory/auto-apply reviewers (hook, impact, emotion, structure,
wordsmith, grammar, seo, link-opportunity, fact-checker, bullshit-detector,
meta-content) aren't covered yet — same pattern, lower priority since they
don't block convergence.
...
signal (`scoreText()` in `.claude/skills/human-tone/eval/tone-grader.ts`), not
an LLM agent. Its `.expected.md` has a copy-pasteable `npx tsx` snippet instead
of an agent-dispatch step — run that directly.
