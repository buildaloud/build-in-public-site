...
export type GraderOpts = { offline?: boolean };

// Allowed hosts + relative prefixes — sourced from PLAYBOOK.md §5 + src/data/projects.ts
const ALLOWED_HOSTS = new Set(['buildaloud.ai', 'marketplace.buildaloud.ai', 'td.buildaloud.com']);
const ALLOWED_RELATIVE_PREFIXES = ['/blog/', '/projects'];

// Banned strings (case-insensitive)
const BANNED = ['change-factory', 'change factory'];
...
function isAllowedLink(link: string): boolean {
  if (ALLOWED_RELATIVE_PREFIXES.some(p => link.startsWith(p))) return true;
  try {
    return ALLOWED_HOSTS.has(new URL(link).host);
  } catch {
    return false;
  }
}

export function gradeInternalLinks(brief: Brief): GraderResult {
  if (brief.internalLinks.length < 2) {
    return { id: 'internal-links', pass: false, message: 'fewer than 2 internalLinks' };
  }
  const bad = brief.internalLinks.filter(l => !isAllowedLink(l));
  if (bad.length > 0) {
...
export function gradeBannedTerms(brief: Brief): GraderResult {
  const fields = [
    brief.topic,
    brief.targetKeyword,
  ...
  const found = BANNED.filter(t => haystack.includes(t));
  if (found.length > 0) {
...
export async function runGraders(raw: unknown, opts?: GraderOpts): Promise<GraderResult[]> {
  ...
  return [
    ...
    gradeInternalLinks(brief),
    gradeSlugKebab(brief),
    gradeBannedTerms(brief),
