...
How Build Aloud decides **what publishes when**. Owned by the content pipeline
(`.claude/skills/content-pipeline`) — every post is scored and placed by this
model, and the model is re-tuned as we learn. Chad set the policy 2026-07-11.
...
- **Bucket A — our work** (anything Chad built/decided/learned in a Claude
  session; today's changes). **Always front-loaded**, ordered by recency. It's
  time-sensitive: a build-in-public post about what we shipped today loses value
...
top-scored post from the Bucket B pool.** So on any day Chad ships fresh work, it
front-loads; on any day he doesn't, the best evergreen backfills — the cadence
never stalls and the highest-impact evergreen always surfaces first. Low-scoring
Bucket B posts are the long tail that sinks to fill future gaps.
...
Edge call: a post that's *our lived experience of an external thing* (Chad
exploring local LLM hosting, Chad's own chat with Gemini) is **Bucket A** — the
spine is our work, the external thing is the occasion.
...
1. **At assemble time** (pipeline Step 10.5) the orchestrator scores the post,
   ...
   `src/data/seo-predictions.json`: the four sub-scores, total, bucket, chosen
   slot, and a one-line rationale.
2. **At `stats:pull` time** (or any rebalance) reconcile: join predictions to
   actual `stats.json` (views / impressions / clicks / position for that slug),
   compute the gap, and note which factor was off (e.g. "opportunity
   over-weighted — ranked but no volume").
...
Ledger shape (`src/data/seo-predictions.json`):
