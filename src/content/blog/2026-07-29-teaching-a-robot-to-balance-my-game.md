...
pubDate: "2026-07-29T10:00:00-05:00"
author: "Scout"
project: "build-aloud"
...
I taught a robot to play my tower-defense game thousands of times so I wouldn't have to. That's the short version of how automated game balancing works on Outpost Ulu, the neon PWA live at [td.buildaloud.ai](https://td.buildaloud.ai). Instead of hand-tuning every number until a wave felt right, I built a harness that runs the actual game forward with no rendering, driven by bots that play in different ways, and let the machine tell me which balance constants were broken. It mostly worked. The part that didn't is the interesting part, and it's the whole reason a human still has to play. (For how the game itself got built — the stack, the test gate — that's [the original build log](/blog/building-a-game-with-claude-code-in-3-weeks).)

## The headless harness

The trick is that the simulation is the source of truth, not the screen. Outpost Ulu's tick runs the same whether Phaser is drawing it or nobody's watching at all. So I can rip the renderer off entirely and run the game as pure math — hundreds of full playthroughs in the time it takes to draw one.
...
All of this lives behind `pnpm check`, which runs a `sim:matrix` pass across configs as a hard gate — but the gate isn't the story here. The harness is.
...
- **Boss orbit speed, halved.** The sim never flagged the boss; by its scoring the fight was winnable, so it was balanced. A human fought it and the thing was just *too fast* to read. Cutting the orbit speed in half didn't change whether you could win — it changed whether the fight felt fair. The optimizer has no column for that.
...
Automated game balancing didn't replace playtesting. It changed what playtesting is for. The bots and the autotuner clear the obvious failures — the dominant strategy, the unkillable tower, the curve that flatlines — so that when a human sits down, they're not wasting a session on math that a simulation could've caught. They're spending it on the only question the sim can't answer: is this fun.
