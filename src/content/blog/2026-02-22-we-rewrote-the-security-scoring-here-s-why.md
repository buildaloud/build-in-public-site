...
The problem surfaced almost immediately. Our first batch of audits flagged several skills as "high danger" because they used Bash or spawned subprocesses. But that's almost useless as a signal. Most legitimate skills touch a shell for ordinary reasons: a build, a git call, a package install. Flag everything that touches a shell and the whole catalog looks dangerous.
...
The new system is called AST v1.0, short for Agent Skill Threats. It's a fixed, versioned taxonomy of 10 threat types, inspired by OWASP Top 10. It focuses on **what bad outcome the author intended**, not what tools the skill happens to use.
...
Each finding in an audit maps to exactly one of these types. That gives us a common vocabulary for describing threats across hundreds of different skills. It also gives users a way to filter on the specific risks they care about. An agent that handles financial transactions has different concerns than a code editor. AST types let you query for what actually matters to you.
...
**Misuse Surface**: How likely is an agent to go off the rails with this skill? This focuses on `negligent` and `accidental` findings: cases where the code means well but isn't guarded. Unconfirmed delete operations and unbounded autonomy get extra weight here.
...
One other thing the old model couldn't do: tell the difference between three kinds of skill authors. One trying to hurt you. One who was careless. One who built something genuinely risky by accident.
...
The marketplace will eventually expose the exposure score and AST type breakdowns in the UI. Then you can search for "show me everything with a misuse surface above 20" or "filter out anything with an AST-02 finding." That's the endgame: a catalog you can query by risk level instead of reading line by line.

For now: re-audit running, scores incoming, taxonomy versioned and documented.

---

*Source: `docs/plans/2026-02-22-security-audit-taxonomy-design.md` in the skills-marketplace repo. The full AST v1.0 spec lives there, along with the JSON schema and scoring formulas.*
